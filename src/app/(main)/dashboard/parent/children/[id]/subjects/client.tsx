...
import { MessageSquare, BookOpen, FileQuestion, TrendingUp, Mail, Loader2 } from "lucide-react";
...
interface ChildSubjectsClientProps {
  initialSubjects: Subject[];
  initialChild: any;
}

export default function ChildSubjectsClient({ initialSubjects, initialChild }: ChildSubjectsClientProps) {
  const { toast } = useToast();
  const [subjects] = useState<Subject[]>(initialSubjects);
  const [child] = useState<any>(initialChild);
  const [isLoading] = useState(false); // Data is now pre-fetched
